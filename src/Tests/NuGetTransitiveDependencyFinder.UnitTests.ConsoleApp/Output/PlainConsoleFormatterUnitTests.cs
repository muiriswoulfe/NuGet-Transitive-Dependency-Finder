...
public class PlainConsoleFormatterUnitTests
{
    ...
    /// <summary>
    /// Tests that when
    /// <see cref="ConsoleFormatter.Write{TState}(in LogEntry{TState}, IExternalScopeProvider, TextWriter)"/> is called
    /// with a valid log level, the correct string is logged.
    /// </summary>
    ...
    /// Tests that when
    /// <see cref="ConsoleFormatter.Write{TState}(in LogEntry{TState}, IExternalScopeProvider, TextWriter)"/> is called
    /// with each <see cref="LogLevel"/> present in the enumeration, the appropriate conversion is performed and
    /// <see cref="InvalidEnumArgumentException"/> is never thrown.
    /// </summary>
    [AllCulturesFact]
    ...
    /// Tests that when
    /// <see cref="ConsoleFormatter.Write{TState}(in LogEntry{TState}, IExternalScopeProvider, TextWriter)"/> is called
    /// with an invalid log level, a <see cref="InvalidEnumArgumentException"/> is thrown.
    /// </summary>
    [AllCulturesFact]
    public void Write_WithInvalidLevel_ThrowsInvalidEnumArgumentException()
    {
        ...
        _ = action
            .Should().Throw<InvalidEnumArgumentException>().WithMessage(
                "The value of argument 'logLevel' (100) is invalid for Enum type 'LogLevel'. (Parameter 'logLevel')")
