...
namespace NuGetTransitiveDependencyFinder.Wrappers
{
    ...
    internal interface IProcessWrapper
    {
        ...
        /// <param name="outputDataReceived">An event for handling when the process writes to its redirected standard
        /// output stream.</param>
        /// <param name="errorDataReceived">An event for handling when the process writes to its redirected standard
        /// error stream.</param>
        /// <exception cref="InvalidOperationException">No file name was specified in the <see cref="ProcessWrapper"/>
        /// component's <paramref name="startInfo"/>, or the <see cref="ProcessStartInfo.UseShellExecute"/> member of
        /// the <paramref name="startInfo"/> property is <see langword="true"/> while
