...
public static class ServiceCollectionExtensions
{
    ...
    /// <param name="loggingBuilderAction">The <see cref="Action{ILoggerBuilder}"/>, which defines the logging action
    /// for both the current application and the NuGet Transitive Dependency Finder library.</param>
    /// <returns>The <see cref="IServiceCollection"/> passed to the method with the additional dependencies
    /// included.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="value"/> or <paramref name="loggingBuilderAction"/> is
